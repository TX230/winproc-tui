...
- `footer`: Uses one content row to draw the focused panel name in uppercase and its main contextual actions. The focused panel name uses the high-contrast neutral accent, while shortcut keys use muted neutral text. Live and Recording place the common `Ctrl+P Pause` action immediately after the panel name for every focused panel. The predictable Tab focus-cycle shortcut is omitted from the Footer but remains documented in Help and the README. It does not render `App::status`; complete key bindings remain in the Help dialog.
- `header`: Keeps the first row dedicated to the active activity and exceptional state: `LIVE` uses the semantic success color, `REC` uses danger red, and `LOG` uses warning amber. The product name and version are shown in Help instead of consuming persistent header width. Live and Recording append only the exceptional age-bearing `STALE Ns` warning; the normal fresh state has no text. `DISPLAY PAUSED` is a separate badge placed before any log path so a long path cannot hide it. The event loop redraws when the freshness state changes, including while display updates are paused.
- `system_panel`: Draws the top row as `RAM/VRAM`, `NW/DISK`, and `CPUs`. The Live and Recording RAM/VRAM title omits the fixed history capacity, which remains available in Help; Log view appends its actual loaded sample count. RAM/VRAM rows highlight the selected metric and Graph slot state. NW/DISK rows show System Activity metrics and can take focus, select a metric, and assign Graph slots with the same row/number behavior as RAM/VRAM. Graph-slot numbers use the shared amber-foreground marker style without a background fill or bold text. The System Info view is drawn as a top-level modal dialog when requested.
- `cpu_panel`: Draws the compact `CPUS` panel in the top row. It shows average CPU usage, current P/E clock averages in integer MHz when PDH reports them, and a `Per-core Usage` row where P/E groups are labeled on the same line as their per-logical-CPU glyphs. CPU bars use the theme's neutral accent at every utilization level; bar length, glyph height, and the numeric percentage carry magnitude without green-to-red severity semantics. The panel can take focus; `CPU Usage` uses `SystemHistory`, can be assigned to Graph slots, reserves the first two content cells for the Graph slot number, and uses the same shared marker style as the other panels.
- `process_table`: Uses `App::visible_process_row_window(offset, rows)` to build a ratatui `Table`. Its `PROCESSES` title reports the visible row count, All processes / Tracked only state, and active filter as plain separated text; sort state stays in the table header and fixed history capacities live in Help instead. The Tracked-only mouse target is derived from the same title-segment helper used for drawing. Ghost Rows (exited tracked processes) use a muted color and appear after live rows. The cursor row, selected column, and selected-column header share one grayscale surface; their intersection uses a stronger grayscale surface, while live multi-selected rows retain a separate neutral surface. The `>>` cursor and `↑` / `↓` sort markers preserve state recognition without relying on hue. Process column-name text is underlined as a structural cue without bold styling; ordinary header cells share the table background. Cell padding and the adjacent sort arrow remain unmodified. Ordinary live metric changes and the Tracked Total row remain neutral instead of using semantic or selection colors. `Tracked only` and live tracked stars use the subdued tracked-amber foreground; exited tracked stars use muted gray. Graph-slot numbers retain the brighter warning amber. None of these markers use bold or background fills. Byte-valued memory cells use adaptive decimal units only in this overview; their columns reserve 10 cells for a Graph-slot marker, separator, and compact value, except `.NET Heap`, which reserves 11 cells so its sort arrow remains visible. Samples, A/B comparison, clipboard output, sorting, Graph data, and recording logs continue using raw values. Filter matches use warning amber, and Full Path suppresses duplicate highlighting when the process name already matches. The `Full Path` column is rendered as left-aligned text, takes extra table width when visible, and is shortened from the start when needed. While the user is actively moving the process cursor, sample refreshes briefly preserve the current row order to avoid periodic cursor jumps from metric-driven resorting.
- `details_panel`: The complete Graph area has one shared controls row labeled `GRAPHS` for the visible time span, cursor and A/B times, Fit-all mode, and Y-min lock. Each slot uses one outer frame titled `GRAPH#n` with its target and metric; the Graph (`Chart` widget) and synchronized Samples table share that frame and are separated by one divider. The active slot title is emphasized and inactive titles are muted. Time span, cursor age, and A/B points are shared across slots, while Y-axis scale, sample availability, and value labels are evaluated per slot. Drawing and mouse hit testing derive the shared-controls, slot-title, Graph, divider, and Samples rectangles from the same `ui::layout` helpers. The Graph shows history for the current slot across `graph_time_span_seconds`. It supports Y-min lock to 0, A/B markers, selected cursor, and offset movement with Ctrl+Arrow or mouse drag. The selected cursor guide uses a dedicated low-contrast neutral gray because it represents a navigation position rather than status; its value label remains high-contrast neutral. A/B marker letters use amber because they are explicit user-set comparison points. Samples deltas use muted neutral text regardless of direction, with signed text carrying the direction; the selected row promotes an available delta to the normal text color, while unavailable `--` values remain muted. The selected cursor and A/B vertical guides stop one terminal row above the Graph X-axis. A/B labels are drawn on the X-axis line, separate from the selected cursor value label. Ctrl+Arrow pans by about one eighth of the current time span; mouse drag clamps the offset so at least one sample from the active series remains in the visible range. Fit-all mode is not cleared by drag attempts. Live graph scrolling stops only when the Graph visible range is moved away from the live edge by Ctrl+Arrow or graph mouse drag; moving the cursor or setting an A/B point does not freeze the Graph visible range. While stopped, sampling updates preserve the absolute visible time window instead of sliding it forward.
...
`theme` manages the neutral-first `Dark` / `Light` color definitions and theme index switching. Both themes separate high-contrast neutral foreground accents from the softer generic focus, process-table selection / intersection, row highlight, and multi-selection surfaces. They also keep the subdued tracking amber distinct from the brighter warning amber; green / amber / red remain semantic status colors.
...
- `build_runtime_config` resolves string column names and preset names into enums, builds `SamplingOptions`, and returns `RuntimeConfig`. With no saved selection, the Default preset enables every column in `MetricColumn::ALL`; an explicit non-empty `columns` list takes priority. The sampling interval is not part of the configuration because the runtime interval is fixed at 1 second.
