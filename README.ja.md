...
ターミナルから起動し、メモリ・ハンドル・GUI リソース・GPU メモリ・I/O などの現在値と時間変化を確認できます。最大 4 つの Graph、A/B 比較、JSON Lines 形式のログ記録と保存ログ表示を備えており、開発・検証時のリソース挙動や日常的なトラブルシュートを短時間で把握するのに向いています。Process Explorer や System Informer のように網羅性で勝負するのではなく、「いま見ているプロセスが、どんなリソースをどのタイミングでどの程度使っているか」を素早く掴むことに振り切ったツールです。Rust/Ratatui で作られています。
...
Dark / Light テーマは、フォーカスや選択を落ち着いたグレーの背景とカーソル / ソート記号で示す Neutral-first 配色です。Processes テーブルでは、サンプルごとの増減を色分けせず、通常のメトリクス値と Tracked Total 行を中立色で表示します。カラム名は太字やヘッダー行全体の背景帯を使わず下線で区別します。カーソル行、選択列、選択列のカラム名は同じグレースケール背景に統一し、行と列の交差セルだけを一段強いグレーで表示します。CPU 利用率バーは緑から赤へのグラデーションを使わず、テーマの中立的な accent 色で統一し、バー長・字形・数値で利用率を示します。Graph のカーソル線は muted グレー、通常の Samples Delta は中立色で表示し、増減方向は符号で示します。緑は `LIVE` と操作成功、落ち着いたアンバーは `Tracked only` と追跡中の★、より明るい警告アンバーは Graph スロット・フィルター一致・A/B マーカー・`LOG`・一時停止 / stale・警告に使用します。赤は `REC`・危険・エラーに限定します。ショートカットキーは中立色のままにして、状態色を判別しやすくします。
Processes のタイトルには、表示行数、All processes / Tracked only、適用中のフィルターを簡潔に表示します。ソートカラムと方向はテーブルヘッダーだけに表示します。固定の履歴保持数は常設パネルのタイトルには表示せず Help で確認でき、Log view では読み込んだ実サンプル数を表示する場合があります。
...
通常、ヘッダーには `LIVE` または `REC` だけを表示し、製品名とバージョンは Help または `--version` で確認できます。正常なサンプル取得が 3 秒間なければ、次の取得成功まで `STALE Ns` を追加します。`DISPLAY PAUSED` は表示中のスナップショットだけを固定し、サンプリングと記録は継続します。保存ログ表示では `LOG` と表示し、ライブサンプリングの鮮度は表示しません。
...
`f` のような 1 文字キーは、フォーカス中のパネルによって動作が変わります。常設パネルの見出しと Footer の現在パネル名は、`PROCESSES`、`CPUS`、`GRAPHS`、`GRAPH#n` のように大文字で表示します。Live / Recording ではパネルが変わっても `Ctrl+P Pause` を表示し、表示停止を利用できない Log view では省略します。推定しやすい Tab のフォーカス移動はフッターから省略します。下の表にはパネルごとの全操作を掲載しています。
