...
Auto 配置は、カードの最低幅を保てる場合は 2 列、保てない場合は 1 列を使います。2 列では左上、右上、左下、右下の行優先で並びます。Graph が 1 つだけなら横幅全体を使い、奇数件の最後は右側が空きます。Graph 行は縦スクロールでき、`Up` / `Down`、カードのクリック、マウスホイール、スクロールバーで全件へ移動できます。
