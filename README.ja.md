...
グラフ化可能なメトリクスセルでは、`Space` とダブルクリックのどちらも選択中の Graph だけを追加 / 削除します。Process または PID セルでは、同じ操作でそのプロセス名を Tracking List に追加 / 削除します。MEM、GPU、NW/DISK、CPU パネルの選択中メトリクスでは Graph 操作のままです。`PROCESSES` とコンパクトなシステムパネルでは、Graph 登録済みの値を緑、アクティブな Graph の値を太字で表示し、Graph スロット番号用の幅は確保しません。
...
1. `PROCESSES` で対象プロセスの Process または PID セルを選び、`Space` を押すかセルをダブルクリックしてプロセス名を Tracking List に登録します。同じ操作で解除できます。`t` は選択カラムに関係なく使える直接の切り替え操作として残ります。
...
4. `Ctrl+R` を押し、保存先と `1s` / `2s` / `5s` / `10s` の記録間隔を選んで記録を開始します。
...
実行中の .NET 8 / 9 / 10 プロセスはプロセス個体ごとに初回検出され、Tracking List に登録しなくても、マネージドヒープ、Gen0 / Gen1 / Gen2 / LOH / POH のサイズ、GC のコミット済みメモリと断片化、アロケーションレート、世代別の GC レートを表示します。.NET Framework 4.8 では従来のカウンターからヒープ全体、Gen1、Gen2、LOH を表示し、意味を揃えられない Gen0 と存在しない POH は `--` のままです。非対応、アクセス不可、または取得できない値も `--` になります。ログへ記録されるのは引き続き追跡中のプロセスだけです。
記録開始ダイアログには対象名の件数が表示され、その Tracking List がセッション終了まで固定されます。記録中は `t`、`Ctrl+T`、Process/PID セルからの Tracking List 操作を使用できません。表示だけを切り替える `Shift+T` の Tracked-only は引き続き使用できます。

Tracking Lists ダイアログでは、名前付きプロセスリストのロード、保存、名前変更、削除を行えます。`Empty (default)` は作業中リストだけを空にし、Tracked-only は変更しません。`Tracking List startup` は `Resume last`、`Choose list`、`Start empty` を選ぶ左寄せの枠付きラジオグループです。`Tab` でフォーカスし、`Left` / `Right` / `Space` で変更して、`Enter` または `Esc` でダイアログを閉じます。リストのロードによって除外対象の保持履歴が失われる場合は確認を表示します。追跡名を削除する際にこの確認が必要な場合は、`Enter` で削除し、`Esc` でキャンセルします。ダイアログ内の全操作は、実行中に `F1` または `?` で確認できます。

起動時の扱いを `Choose list` に設定すると、起動画面で `Up` / `Down` を使って Tracking List を選びます。`Enter` で選択したリストを使って開始し、`Esc` で初回サンプルを取得せずに終了します。

プロセス、システムメトリクス、Samples の選択行で `Ctrl+C` を押すと、Issue や調査メモへ貼り付けやすいプレーンテキストをコピーできます。System Info では、小さいターミナルで画面外になる項目も含め、環境情報全体をコピーします。長時間の調査では `.log` ファイルを残し、`Ctrl+L` から再度開けます。
...
| `Space`             | 選択中の Process/PID を追跡 / 解除するか、選択中メトリクスの Graph を追加 / 削除する。 |
...
- **モニタ**: 2 ページのメモリ負荷、GPU アダプター別の GPU / Encode / Decode 負荷とメモリ、ネットワークとディスク、CPU パネル、`WS Shrbl`、自動検出した最新 .NET ランタイム指標、.NET Framework 4.8 の一部ヒープ指標を含むプロセス別メトリクスを表示。ソート、列選択、フィルター、ジャンプ検索で対象を絞り込めます。
...
- **プロセス調査**: 選択中プロセスのメトリクス、実行ファイルとロード済み .NET ランタイムの情報、現在開いているファイルを、レスポンシブなタブ式 Process Info ダイアログで確認できます。
...
README には主要操作のみを掲載します。**実行中に** `F1` **または** `?` **を押すと、現在割り当てられている全キーをヘルプダイアログで確認できます。**
...
| `Ctrl+C`            | 選択行、または System Info の全項目をコピー。       |
| `Ctrl+L`            | ログ一覧を開く。                       |
...
| `Space`             | 選択中の Process/PID の名前を Tracking List に追加 / 削除するか、選択中のグラフ化可能なメトリクスを Graph Workspace に追加 / 削除する。 |
...
System Info には、winproc-tui のバージョン、Windows のバージョン・ビルド・アーキテクチャ、CPU、物理メモリとコミット上限、GPU アダプター容量、各ディスクの空き容量と総容量を表示します。Live、表示一時停止、Log view のいずれでも現在の PC 環境を示します。`Ctrl+C` でラベル付きの全項目をプレーンテキストとしてコピーし、`Enter` または `Esc` で閉じます。
...
| `Shift+Up` / `Shift+Down` | 操作中の Graph を 1 つ前 / 後ろへ移動。      |
| `s`                        | Graph または Samples から並べ替えダイアログを開く。 |
...
Graph Workspace には最大 16 個のカードを順序付きで登録できます。`Up` / `Down`、カードのクリック、マウスホイール、スクロールバーで操作中の Graph を選び、`Shift+Up` / `Shift+Down` で操作中のカードをすぐに移動できます。Graph または Samples で `s` を押すと `REORDER GRAPHS` が開きます。`Up` / `Down` で行を選択し、`Shift+Up` / `Shift+Down` で移動してから、`Enter` で適用、`Esc` でキャンセルします。`Delete` またはカードの `[x]` で Graph を削除します。タイトルの `[-]` / `[+]` とカードの `[x]` は、マウスを重ねると強調表示されます。Samples インスペクタは操作中の Graph に追従し、共通操作は Graph と Samples のどちらからでも使用できます。
...
`Ctrl+R` で記録を開始するか、記録中なら停止確認を開きます。停止確認を表示している間も記録は継続し、`Enter`、`Esc`、`n` では継続、`y` では停止します。1 回の記録は最大 24 時間です。上限に達すると正常終了レコードを書き込み、ログをフラッシュして閉じ、自動的に `LIVE` へ戻ります。記録を開始するには Tracking List に 1 件以上の名前が必要です。ログは JSON Lines として保存されます（拡張子 `.log`）。

記録開始時に `1s`、`2s`、`5s`、`10s` から集約間隔を選べます。既定値は `1s` です。Live の収集と履歴は常に 1 秒間隔のままで、長い間隔を選ぶと、その個数の 1 秒サンプルを算術平均した 1 フレームをログに書き込みます。取得できなかった値や存在しなかったプロセスを 0 として扱わず、各プロセスを PID・名前・開始時刻の組み合わせごとに平均します。長い間隔はログサイズと Log view のロード負荷を減らしますが、短時間の CPU/GPU スパイクやメモリ、ハンドル数のピークを平滑化します。

各出力フレームには MEM、GPU アダプター別メトリクス、平均 CPU 使用率、System Activity などのシステム指標と、Tracking List に一致する実行中プロセスが記録されます。一致するプロセスがその集約区間に存在しない場合も、システム指標は記録され、プロセス一覧は一致するプロセスが現れるまで空になります。停止、アプリ終了、24 時間上限のいずれでも、端数の集約区間を先に書き出してから正常終了レコードを書き込みます。

記録開始ダイアログでは保存先と集約間隔を確認し、セッション終了まで固定される Tracking List の対象件数、JSON Lines 形式、最大 24 時間の上限も項目ごとに表示します。`Tab` / `Shift+Tab` で操作対象を移動し、集約間隔にフォーカスがあるときは `Left` / `Right` で値を変更します。保存先にはログのファイル名まで指定する必要があり、ディレクトリだけでは記録を開始できません。存在しない親ディレクトリは自動作成します。`Enter` で開始、`Esc` でキャンセルし、パスにフォーカスがあるときは `Ctrl+Space` でディレクトリ名を補完できます。記録中は `t`、`Ctrl+T`、Process/PID セルでの `Space` またはダブルクリックを拒否して案内を表示します。Tracked-only 表示だけを変える `Shift+T` は引き続き使用できます。ログの作成、書き込み、フラッシュに失敗した場合は記録を終了し、部分ログを残してエラーダイアログを表示します。終了時のフラッシュに失敗した場合はアプリの終了を取り消します。Log view 中は記録を開始できず、記録中は Log view を開けません。

`Ctrl+L` でログ一覧を開きます。前回の記録ディレクトリがあればそこ、なければカレントディレクトリの `*.log` のファイル名をコンパクトな一覧に表示します。`Dir` 行で検索中のディレクトリを確認できます。`Up` / `Down` でファイルを選び、`Enter` で開き、`d` で別ディレクトリを指定し、`r` で更新し、`Esc` で閉じます。ログの読み込み中は一覧に `Opening <filename>...` と表示し、完了するまで追加のオープン操作を無視します。全キーバインドはダイアログ最下段に表示します。選択したログを開くと表示が `LOG` に切り替わり、Processes / Graph / Samples / A/B 比較で過去のセッションを調査できます。
Log view は再生機能ではありません。Processes は記録の最終値を表示し続け、Graph、Samples、Process Info で記録済みメトリクスの履歴を確認します。ヘッダーには `10s AVG` のように集約間隔を表示します。Graph は記録時刻をそのまま使い、プロセスやメトリクスが欠けた区間を線で接続しません。集約ログの Samples では、`Max` は集約値の最大として表示し、`MA5 (5×10s)` のように MA5 が記録済み 5 フレームの平均であることを示します。Process Info の静的情報には記録済み項目を使い、記録されていない項目は `--` と表示します。`Esc` で Live 表示へ戻ります。
