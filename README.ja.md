...
登録済みの項目で `Space` をもう一度押すと、その Graph だけを削除します。登録済み項目のダブルクリックは削除せず、既存の Graph を表示します。RAM / VRAM と NW/DISK の選択中メトリクス、CPUS の CPU Usage でも同じ操作を使えます。Graph に登録した項目には、対応するスロット番号を表示します。
...
Tracking Lists ダイアログでは、名前付きプロセスリストのロード、保存、名前変更、削除を行えます。`Empty (default)` は作業中リストだけを空にし、Tracked-only は変更しません。リストのロードによって除外対象の保持履歴が失われる場合は確認を表示します。ダイアログ内の全操作は、実行中に `?` で確認できます。
...
Graph Workspace には最大 16 個のカードを順序付きで登録できます。`Up` / `Down`、カードのクリック、マウスホイール、スクロールバーで操作中の Graph を選び、`Delete` またはカードの `[x]` で削除します。Samples インスペクタは操作中の Graph に追従し、共通操作は Graph と Samples のどちらからでも使用できます。

複数 Graph は表示時間幅、カーソル、選択時刻、A/B 点を共有しますが、Y 軸とサンプルの有無は Graph ごとに独立しています。A/B 値、クリップボード出力、記録ログは正確な値を維持し、選択時刻と完全一致するサンプルがない場合は近傍値で補わず `--` を表示します。
