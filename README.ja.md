...
Auto 配置は、カードの最低幅を保てる範囲で最大 3 列を使います。2 列または 3 列を明示した場合も、必要な幅がなければ列数を減らします。カードは行優先で並びます。Graph が 1 つだけなら横幅全体を使い、最後の行に空きがある場合は残りの位置を空けます。Graph 行は縦スクロールでき、`Up` / `Down`、カードのクリック、マウスホイール、スクロールバーで全件へ移動できます。
