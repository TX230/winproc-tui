...
Auto layout uses up to three columns while preserving the minimum card width. Explicit two- and three-column layouts use fewer columns when the requested count does not fit. Cards are arranged in row-major order. A single Graph always uses the full width, and an incomplete final row leaves its remaining positions empty. Graph rows scroll vertically and remain reachable through `Up` / `Down`, card clicks, the mouse wheel, and the scrollbar.

The single Samples inspector always follows the active Graph. It appears to the right when width allows, below the Graph cards when only height allows, and temporarily collapses when neither placement is readable. Restoring terminal space brings back only a temporarily collapsed inspector; one explicitly hidden with `v` stays hidden. Multi-column Graphs and Samples can be shown together when enough width is available.
